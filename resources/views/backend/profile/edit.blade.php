    <x-backend-layout title="Editer le profil" width="max-w-2xl">
        ...
        <x-image-uploader :asset="$user->portrait" :resource="$user->id" :resourceType="'user'" group="portrait:unique" />
        ...
        <form action="{{ route('console.profile.update') }}" method="post" class="mt-6">
            ...
            <div class="mt-4">
                ...
                <textarea id="description" name="description" aria-label="Description"
                    class="resize-none w-full border border-gray-300 mt-0.5 focus:ring-brand-600 focus:border-brand-600" rows="3">{{ $user->bio }}</textarea>
