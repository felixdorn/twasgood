...
const tabs = [
    {
        ...
        href: route('console.articles.index', {state: PublicationState.Published}),
        current: props.state === PublicationState.Published,
    },
    {
        name: `Brouillons (${props.unpublished_count})`,
        href: route('console.articles.index', {state: PublicationState.Unpublished}),
...
<template>
    <Connected title="Articles">
        ...
        <masonry-wall v-if="articles.length > 0" :gap="16" :items="articles" :min-columns="3"
            ...
            <template #default="{ item, index }: { item: Article, index: number }">
                <Link :href="route('console.articles.edit', { article: item.id })"
                    ...
                    <img v-if="item.banner && item.banner.path !== 'uploads/thumbnails/'"
                         loading="lazy"
        ...
        <EmptyState
            v-else
            :action-href="route('console.articles.create')"
