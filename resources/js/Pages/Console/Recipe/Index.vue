...
const tabs = [
    {
        ...
        href: route('console.recipes.index', {state: PublicationState.Published}),
        current: props.state === PublicationState.Published,
    },
    {
        name: `Brouillons (${props.unpublished_count})`,
        href: route('console.recipes.index', {state: PublicationState.Unpublished}),
...
<template>
    <Connected title="Recettes">
        ...
        <masonry-wall v-if="recipes.length > 0" :gap="16" :items="recipes" :min-columns="3"
                      class="mt-6 w-full -m-4">
            <template #default="{ item, index }: { item: Recipe, index: number }">
                <Link :href="route('console.recipes.edit', { recipe: item.id })"
                    ...
                    <img v-if="item.banner?.path && item.banner?.path !== 'uploads/thumbnails/'"
                         loading="lazy"
                         :src="`/storage/${item.banner.path}`" class="rounded-t-xl" height="507"
                         width="907"/>
                    <div v-else
                    ...
                    <div class="px-5 py-3">
                        <h3 class="md:flex items-center justify-between">
                            <span class="text-lg group-hover:underline">{{ item.title }}</span>
                        </h3>
        ...
        <EmptyState
            v-else
            :action-href="route('console.recipes.create')"
            :action-icon="DocumentPlusIcon"
            :icon="DocumentMagnifyingGlassIcon"
            action-title="Nouveau brouillon"
