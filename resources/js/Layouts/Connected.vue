...
<template>
    ...
    <div>
                        ...
                        <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-brand-700 pt-5 pb-4">
                            ...
                            <div class="mt-5 h-0 flex-1 overflow-y-auto">
                                <nav class="space-y-1 px-2">
                                    <Link v-for="item in navigation" :key="item.name" :class="[
                                        item.current
                    ...
                    <nav class="flex-1 space-y-1 px-2 pb-4 px-4">
                        <ul class="space-y-6">
                            ...
                            <li v-for="section in $page.props.navigation">
                                <div class="text-brand-200 leading-6 font-semibold text-xs">
                                    {{ section.name }}
                                </div>

                                <ul role="list" class="mt-1">
                                    <li v-for="item in section.items">
                                        <Link :key="item.name" :href="item.url" :class="[
                                            item.active
                                                ? 'bg-brand-800 text-white'
                                                : 'text-brand-100 hover:bg-brand-600',
                                        ...
                                        <component :is="getIcon(item.icon)"
                                            class="mr-3 h-6 w-6 flex-shrink-0 text-brand-300" aria-hidden="true" />
                                        {{ item.name }}
                                        </Link>
                                    </li>
                                </ul>
        ...
        <div class="flex flex-1 flex-col md:pl-64">
            <div class="z-10 flex h-16 flex-shrink-0 bg-white" :class="[
                searchResults.length || searchQuery.length ? '' : 'shadow',
            ]">
                <button
                    ...
                    <div class="flex flex-1">
                        <form action="#" class="flex w-full md:ml-0" method="GET">
                            ...
                            <div class="relative w-full text-gray-400 focus-within:text-gray-600">
                                ...
                                <input id="search-field" @input="search($event.target.value)"
                    ...
                    <div v-if="searchResults.length">
                        ...
                        <ul class="mt-4 space-y-6">
                            <li v-for="result in searchResults">
                                <Link :href="route('console.recipes.edit', {
                                    recipe: result,
                                })
                                    " class="flex items-center hover:bg-gray-100 py-2 px-4 -my-2 -mx-4 rounded-xl">
                                <img :src="result.banner.url" :alt="result.banner.alt" :title="result.banner.alt"
                                    class="w-12 h-12 object-cover object-center rounded-xl" />

                                <div class="ml-4">
                                    <span class="font-semibold block -mt-px">{{ result.title }}</span>
                                    <span class="block text-gray-700 max-w-lg truncate mt-0.5">{{ result.description
                                        }}</span>
                                </div>
                                </Link>
                            </li>
