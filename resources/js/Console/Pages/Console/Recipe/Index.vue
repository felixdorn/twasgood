...
<template>
    <Connected title="Recettes">
        <template #header>
            <a :href="route('console.recipes.create')" :preserve-scroll="true" :preserve-state="true" :only="['modal']">
        ...
        <div v-if="recipes.length > 0" :gap="16" :items="recipes" :min-columns="3"
            class="mt-6 w-full -m-4 columns-3 xl:columns-4 gap-x-4">
            <Link :href="route('console.recipes.edit', { recipe: item.id })" v-for="item in recipes"
