    ...
    <Combobox v-model="selected" as="div" :class="[first ? '' : 'mt-4']">
        ...
        <div class="relative" :class="hideLabel ? '' : 'mt-1'">
            <ComboboxInput :display-value="(model) => model?.[searchProperty]"
                class="py-2 pr-10 pl-3 w-full bg-white rounded-xl border border-gray-200 shadow-sm sm:text-sm focus:ring-1 focus:outline-none focus:border-brand-500 focus:ring-brand-500"
                v-bind="$attrs" @change="query = $event.target.value" />
            <ComboboxButton class="flex absolute inset-y-0 right-0 items-center px-2 rounded-r-xl focus:outline-none">
                <ChevronUpDownIcon aria-hidden="true" class="w-5 h-5 text-gray-400" />
            </ComboboxButton>

            <ComboboxOptions v-if="filtered.length > 0"
                class="overflow-auto absolute py-1 mt-1 w-full max-h-60 text-base bg-white rounded-xl ring-1 ring-black ring-opacity-5 shadow-lg sm:text-sm focus:outline-none z-[1000]">
